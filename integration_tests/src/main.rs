...
use crate::types::{HumanReadableAccount, StakePoolInfoResponse};
use near_units::*;
use workspaces::network::Sandbox;
use workspaces::prelude::*;

const NEAR_LIQUID_TOKEN_WASM_FILEPATH: &str =
    "/Users/bharath12345/stader-work/near-liquid-token/res/nearX.wasm";
...
// Return type is the worker, nearx liquid token contract and stake pool contract with 3 users
async fn setup_sandbox_workspace() -> anyhow::Result<(
    ...
    near_pool_contract
        .call(&worker, "new")
        .args_json(json!({
                "owner_account_id": near_pool_contract.id(),
                "operator_account_id": near_pool_contract.id(),
        }))?
        .transact()
        .await?;
    ...
    // Add the stake pool
    println!("Adding stake pool");
    near_pool_contract
        .call(&worker, "add_staking_pool")
        .args_json(json!({ "account_id": stake_pool_contract.id() }))?
        .transact()
        .await?;
    println!("Successfully Added the stake pool!");
...
async fn get_stake_pool_info(
    worker: &Worker<Sandbox>,
    near_pool_contract: &Contract,
) -> anyhow::Result<StakePoolInfoResponse> {
    near_pool_contract
        .call(worker, "get_sp_info")
        .args_json(json!({ "inx": 0 }))?
        .view()
        .await?
        .json::<StakePoolInfoResponse>()
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let (worker, near_pool_contract, stake_pool_contract, user1, user2, user3) =
    ...
    println!("Fast forward 50400 blocks");
